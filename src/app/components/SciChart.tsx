...
import { ELegendOrientation, ELegendPlacement, EPieType, EllipsePointMarker, FastLineRenderableSeries, GradientParams, NumberRange, NumericAxis, PieSegment, Point, SciChartJsNavyTheme, SciChartPieSurface, SciChartSurface, SweepAnimation, XyDataSeries } from 'scichart';
...
const SciChart = () => {
  ...
  useEffect(() => {
    ...
    let filtered = [];
    if (searchTerm) {
      const fliterbySelectedCounty = data.filter((item) => String(item.country).toLowerCase() === searchTerm.toLowerCase() && item.year >= 2015)
      const fliterbySearch = data.filter((item) => String(item.country).toLowerCase().includes(searchTerm.toLowerCase()) && item.year >= 2015)
  ...
  async function initSciChart(Data: any[]) {
    ...
    const { sciChartSurface, wasmContext } = await SciChartSurface.create("scichart-root", {
      ...
      title: county + " Maize crop Yield 2021 and 2022",
    ...
    sciChartSurface.xAxes.add(new NumericAxis(wasmContext, { axisTitle: "Years", growBy }));
    sciChartSurface.yAxes.add(new NumericAxis(wasmContext, { axisTitle: "Yield", growBy }));
  ...
  return (
    <><div className="container mx-auto p-4">
      <h1 className="text-3xl font-semibold mb-4">Search County See Their Year Maize crop Yield From  2015 and 2022</h1>
      <input
        type="text"
        placeholder="Search..."
      ...
      <ul className="divide-y divide-gray-300">
        {searchResults.map((item, index) => (
          <li onClick={() => selectCounty(item)} key={index} className="p-4 text-white">
            {item}
          </li>
        ))}
      </ul>
      <div className='flex place-items-center'>
        <div className='rounded' id="scichart-root" style={{ width: '100%', height: '400px' }} />
      </div>
